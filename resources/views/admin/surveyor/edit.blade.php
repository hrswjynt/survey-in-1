			...
			<div class="col-8  p-5 shadow">
				<form method="POST" action="/surveyor/edit">
					...
					<div class="input-group mb-3">
						...
						<select class="form-select @error('area') is-invalid @enderror" id="area" name="area">
							@foreach ($kabupaten as $item)
								<option value="{{ $item->id }}" {{ ($profile->kabupaten_id==$item->id)?'selected':'' }}>{{ $item->nama }}</option>
							@endforeach
						</select>
						@error ('area')
						<div class="alert alert-danger">
					...
					<div class="mb-3">
						...
						<input type="text" class="form-control @error('nomor_telepon') is-invalid @enderror" id="nomor_telepon" name="nomor_telepon" value="{{ $profile->nomor_telepon }}">
						@error ('nomor_telepon')
						<div class="alert alert-danger">
							<ul>
								@foreach ($errors->all() as $error)
